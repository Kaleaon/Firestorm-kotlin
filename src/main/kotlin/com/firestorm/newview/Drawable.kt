...
class XformMatrix {
    ...
    fun updateMatrix() { TODO("GPU: recompute world matrix from position/rotation/scale") }

    fun getMinMax(min: Vector3, max: Vector3) {
        TODO("GPU: compute AABB corners from this transform's extent")
    }
}
...
class Face(
    ...
    fun updateCenterAgent() { TODO("GPU: recompute face centre in agent space") }
    fun clearVertexBuffer() { TODO("GPU: release face vertex buffer reference") }
    fun hasGeometry(): Boolean { TODO("GPU: return true if this face has an allocated vertex buffer") }
    fun renderIndexed() { TODO("GPU: issue indexed draw call for this face") }
    fun verify(): Boolean { TODO("GPU: sanity-check face vertex buffer state") }

    fun setPool(pool: Any?, texture: Any?) { TODO("GPU: attach this face to draw pool $pool with texture") }
    fun setTexture(texture: Any?) { TODO("GPU: assign primary texture to this face") }
    fun setNormalMap(normal: Any?) { TODO("GPU: assign normal-map texture to this face") }
    fun setSpecularMap(specular: Any?) { TODO("GPU: assign specular-map texture to this face") }
    fun setPoolType(type: Int) { TODO("GPU: set draw-pool type classification for this face") }
...
class Drawable(
    ...
    companion object {
        ...
        fun incrementVisible() {
            TODO("GPU: advance the current-frame visible counter; update curPixelAngle from window height / camera FOV")
        }
    ...
    fun init(newEntry: Boolean) {
        ...
        generation = -1
        spatialBridge = null
        TODO("APR: resolve octree entry from VOCache if !newEntry; call setOctreeEntry")
    }

    fun unload() {
        TODO("GPU: call setNoLOD and markForUpdate on the associated VOVolume")
    }

    fun destroy() {
        if (isDead()) numZombieDrawables--
        faces.clear()
        TODO("GPU: check pipeline references in debug; remove from octree")
    }
    ...
    fun isAnimating(): Boolean {
        TODO("Check viewerObject angular velocity and linear velocity flags")
    }
    ...
    fun addFace(te: Any?, texture: Any?): Face {
        ...
        if (isState(UNLIT)) face.setState(Face.FULLBRIGHT)
        faces.add(face)
        face.setPoolType(TODO("GPU: resolve pool type from TE + texture") as Int)
        return face
    }
    ...
    fun updateTexture() {
        ...
        if (getVOVolume() != null) {
            TODO("GPU: mark drawable for REBUILD_MATERIAL in pipeline")
        }
    }

    fun updateGeometry(): Boolean {
        TODO("GPU: call viewerObject.updateGeometry(this) to rebuild vertex buffers")
    }

    fun updateFaceSize(idx: Int) {
        TODO("GPU: resize the vertex/index allocation for face $idx")
    }
    ...
    open fun makeActive() {
        if (!isState(ACTIVE)) {
            ...
            if (viewerObject?.isVolume() == true) {
                TODO("GPU: gPipeline.markRebuild(this, REBUILD_VOLUME)")
            }
            updatePartition()
    ...
    open fun movePartition() {
        TODO("GPU: getSpatialPartition()?.move(this, getSpatialGroup())")
    }

    fun updateXform(undamped: Boolean): Float {
        TODO("GPU: interpolate or snap position/rotation/scale toward target; call updateMatrix(); return dist_squared")
    }
    ...
    private fun updatePartition() {
        if (getVOVolume() == null) {
            ...
            TODO("GPU: gPipeline.markMoved(spatialBridge, false)")
        } else {
            TODO("GPU: gPipeline.markRebuild(this, REBUILD_POSITION)")
        }
    }

    open fun updateDistance(cameraOrigin: Vector3, forceUpdate: Boolean) {
        TODO("GPU: compute distanceWRTCamera from camera origin; account for avatar bounding box; call viewerObject.updateLOD()")
    }
    ...
    open fun shiftPos(shiftVector: FloatArray) {
        ...
        xform.updateMatrix()
        if (isStatic()) {
            TODO("GPU: shift face centers/extents by shiftVector; markRebuild if needed; call shift(shiftVector)")
        } else if (spatialBridge != null) {
            TODO("GPU: spatialBridge.shiftPos(shiftVector)")
        } else if (isAvatar()) {
            TODO("GPU: shift(shiftVector)")
        }
    ...
    open fun setVisible(cameraId: Int, results: MutableList<Drawable>? = null, forSelect: Boolean = false) {
        TODO("GPU: mark this drawable visible for the current frame via octree entry")
    }

    open fun updateSpatialExtents() {
        viewerObject?.let {
            TODO("GPU: call viewerObject.updateSpatialExtents(extents[0], extents[1]); setSpatialExtents; updateBinRadius")
        }
        updateBinRadius()
        if (spatialBridge != null) {
            TODO("GPU: getGroupPosition().splat(0)")
        }
    }

    open fun updateBinRadius() {
        val binRadius = if (viewerObject != null) {
            TODO("GPU: return min(viewerObject.getBinRadius(), 256f)")
        } else {
            min(radius * 4f, 256f)
        }
        TODO("GPU: setBinRadius($binRadius)")
    }
    ...
    fun getPositionAgent(): Vector3 {
        TODO("GPU: convert world position to agent-local space via region offset")
    }
    ...
    fun getVisibilityRadius(): Float {
        ...
        if (isLight()) {
            ...
            if (voVol != null) {
                return TODO("GPU: max(getRadius(), voVol.getLightRadius())")
            }
        }
        return radius
    }
    ...
    fun isVisible(): Boolean {
        TODO("GPU: check octree entry / group visibility flag for current frame")
    }

    fun isRecentlyVisible(): Boolean {
        TODO("GPU: return true if visible in current or previous frame (within MIN_VIS_FRAME_RANGE)")
    }
    ...
    fun getSpatialGroup(): SpatialGroup? {
        TODO("GPU: return (SpatialGroup?) getGroup() from octree entry")
    }

    fun getSpatialPartition(): SpatialPartition? {
        TODO("GPU: return getSpatialGroup()?.getSpatialPartition() or avatar partition for avatars")
    }
    ...
    fun setGroup(group: Any?) {
        TODO("GPU: null out face vertex buffers if group changes and drawable is a volume; call octree setGroup")
    }

    fun removeFromOctree() {
        TODO("APR: detach this drawable from its octree entry; remove active cache entry from region")
    }

    open fun cleanupReferences() {
        faces.clear()
        TODO("GPU: gPipeline.unlinkDrawable(this) or remove from spatial group; removeFromOctree")
    }
    ...
    fun getRenderMatrix(): Any {
        TODO("GPU: return isRoot() ? worldMatrix : parent.worldMatrix")
    }
}
