...
class ReadResponder : CacheResponder() {
    ...
    override fun setData(data: ByteArray?, dataSize: Int, imageSize: Int, imageFormat: Int, imageLocal: Boolean) {
        ...
        TODO("APR: populate formattedImage from raw data bytes")
    }
...
class TextureCache(private val threaded: Boolean) {
    ...
    fun update(maxTimeMs: Float): Int {
        TODO("APR: process completed read/write workers and fire responder callbacks")
    }

    fun purgeCache(locationPath: String, removeDir: Boolean = true) {
        TODO("APR: delete all cached texture files under locationPath")
    }
    ...
    fun initCache(locationPath: String, maxSize: Long, textureCacheMismatch: Boolean): Long {
        TODO("APR: initialize header and body cache directories, read existing entries")
    }

    fun readFromCache(localFilename: String, id: UUID, offset: Int, size: Int, responder: ReadResponder): Long {
        TODO("APR: enqueue local-file cache read worker")
    }

    fun readFromCache(id: UUID, offset: Int, size: Int, responder: ReadResponder): Long {
        TODO("APR: enqueue remote cache read worker for texture id $id")
    }

    fun readComplete(handle: Long, abort: Boolean): Boolean {
        TODO("APR: check if read worker for handle is complete")
    }
    ...
    fun writeToCache(
        ...
        TODO("APR: enqueue cache write worker for texture id $id")
    }

    fun readFromFastCache(id: UUID, discardLevel: Int): Pair<Any?, Int> {
        TODO("APR: synchronously read fast-cache thumbnail for texture id $id")
    }

    fun writeComplete(handle: Long, abort: Boolean = false): Boolean {
        TODO("APR: check if write worker for handle is complete")
    }
    ...
    fun removeFromCache(id: UUID): Boolean {
        TODO("APR: remove texture entry from header and body cache files")
    }
    ...
    fun isInLocal(id: UUID): Boolean { TODO("APR: check if local texture file exists for id $id") }

    fun getLocalFileName(id: UUID): String {
        TODO("APR: compute local filesystem path for texture id $id")
    }

    fun getTextureFileName(id: UUID): String {
        TODO("APR: compute body cache filesystem path for texture id $id")
    }
    ...
    private fun readHeaderCache() { TODO("APR: read header entries file into headerIdMap") }
    private fun clearCorruptedCache() { TODO("APR: remove and reinitialize corrupted cache files") }
    private fun purgeAllTextures(purgeDirectories: Boolean) { TODO("APR: delete all texture body files") }
    private fun purgeTexturesLazy(timeLimitSec: Float) { TODO("APR: evict LRU textures within time budget") }
    private fun purgeTextures(validate: Boolean) { TODO("APR: evict textures to bring cache within size limit") }

    private fun readEntriesHeader() { TODO("APR: read EntriesInfo header from texture.entries") }
    private fun setEntriesHeader() { TODO("APR: initialize in-memory EntriesInfo header") }
    private fun writeEntriesHeader() { TODO("APR: flush EntriesInfo header to texture.entries") }

    private fun openAndReadEntry(id: UUID, entry: CacheEntry, create: Boolean): Int {
        TODO("APR: look up or create header entry for texture id $id")
    }

    private fun updateEntry(idx: Int, entry: CacheEntry, newImageSize: Int, newBodySize: Int): Boolean {
        TODO("APR: update cached entry sizes and timestamp")
    }

    private fun updateEntryTimeStamp(idx: Int, entry: CacheEntry) {
        TODO("APR: refresh LRU timestamp for entry at index $idx")
    }

    private fun openAndReadEntries(entries: MutableList<CacheEntry>): UInt {
        TODO("APR: read all header entries from texture.entries file")
    }

    private fun writeEntriesAndClose(entries: List<CacheEntry>) {
        TODO("APR: write all header entries back to texture.entries file")
    }

    private fun readEntryFromHeaderImmediately(idx: Int, entry: CacheEntry) {
        TODO("APR: synchronously read single entry at byte offset idx*TEXTURE_CACHE_ENTRY_SIZE")
    }

    private fun writeEntryToHeaderImmediately(idx: Int, entry: CacheEntry, writeHeader: Boolean = false) {
        TODO("APR: synchronously write single entry at byte offset idx*TEXTURE_CACHE_ENTRY_SIZE")
    }

    private fun removeEntry(idx: Int, entry: CacheEntry, filename: String) {
        TODO("APR: free header slot and delete body file for entry")
    }

    private fun removeCachedTexture(id: UUID) {
        TODO("APR: remove texture entry and body file for $id")
    }
    ...
    private fun setHeaderCacheEntry(id: UUID, entry: CacheEntry, imageSize: Int, dataSize: Int): Int {
        TODO("APR: create or update header entry for texture $id with sizes")
    }

    private fun writeUpdatedEntries() {
        TODO("APR: flush all pending entry updates from updatedEntryMap to disk")
    }

    private fun updatedHeaderEntriesFile() {
        TODO("APR: rewrite header file with all entries from headerIdMap")
    }
    ...
    private fun openFastCache(firstTime: Boolean = false) {
        TODO("APR: open or create fast-cache file at fastCacheFileName")
    }

    private fun closeFastCache(forced: Boolean = false) {
        TODO("APR: close fast-cache file handle")
    }

    private fun writeToFastCache(imageId: UUID, cacheId: Int, raw: Any?, discardLevel: Int): Boolean {
        TODO("APR: write 16x16 thumbnail pixel data to fast-cache at position cacheId")
    }
...
abstract class TextureCacheWorker(
    ...
    fun doWork(param: Int): Boolean { TODO("APR: dispatch to doRead or doWrite based on param") }
    fun ioComplete(bytes: Int) { bytesRead = bytes }

    private fun startWork(param: Int) {}
    private fun finishWork(param: Int, completed: Boolean) { TODO("APR: notify responder on completion") }
    private fun endWork(param: Int, aborted: Boolean) { TODO("APR: clean up worker state") }
}
...
class TextureCacheLocalFileWorker(
    ...
    override fun doRead(): Boolean {
        TODO("APR: read texture data from local file $fileName")
    }
...
class TextureCacheRemoteWorker(
    ...
    override fun doRead(): Boolean {
        TODO("APR: state-machine read: check local files, then header cache, then body cache file")
    }

    override fun doWrite(): Boolean {
        TODO("APR: state-machine write: update header entry and write body file")
    }
}
